...
def drawSpiral(t, length, color, colorBase):
    #color is a 24 bit  value that is changing a bit each time for a nice color effect
    if length == 0:
        return


    # add 2^10 to the old color module 2^24
    # the modulo 2^24 prevents the color from getting too big.
    ...
    # Now if the newColor is less than the base
